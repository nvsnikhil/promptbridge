document.addEventListener('DOMContentLoaded', function () {
  ...
  function handleApiResponse(response) {
    ...
    if (!response.ok) {
      return response
        .text()
        .then((text) => {
          throw new Error(text || 'An API error occurred.');
        });
    }
  ...
  function renderPrompts(prompts) {
    ...
    if (prompts.length === 0) {
      ...
      prompts.forEach((prompt) => {
        ...
        // Get prompt content from first version (if exists)
        const promptContent =
        ...
        linkElement.innerHTML = `
          ...
          ${tagsHtml}
          <p><strong>Prompt:</strong> ${escapeHtml(promptContent)}</p>
  ...
  // Initial fetch of prompts on page load
  fetchPrompts();
});
